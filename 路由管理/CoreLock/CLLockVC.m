...
- (void)viewDidLoad {
    ...
    UIButton *leftBtn=[[UIButton alloc] initWithFrame:CGRectMake(0, screen_height-30,screen_width/3,15)];
    if (![self hasPwd]) {
        
        [leftBtn setTitle:@"跳过设置?" forState:UIControlStateNormal];
        ...
        [leftBtn addTarget:self action:@selector(leftBtnTap2) forControlEvents:UIControlEventTouchUpInside];
    ...
    UIButton *rightBtn=[[UIButton alloc] initWithFrame:CGRectMake(2*screen_width/3, screen_height-30,screen_width/3,15)];
    
    [rightBtn setTitle:@"修改密码?" forState:UIControlStateNormal];
    [rightBtn addTarget:self action:@selector(rgihtBtnTap) forControlEvents:UIControlEventTouchUpInside];
...
-(void)leftBtnTap2{
    ...
    SecurityProViewController2 *vc=[[SecurityProViewController2 alloc] init];
    vc.title=@"回答问题";
